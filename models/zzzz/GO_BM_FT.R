...
maxiter <- 20
  ...
  bMLE <- soln(maxiter)
	...
	#bMLE <- stats::uniroot(zzzz_MLEeq,lower=leftEndPoint,upper=rightEndPoint, tol = 1e-10, maxiter=2000)$root
...
zzzz_MVF <- function(param,d) {
  ...
  r <- data.frame()
  fail_number <- c(1:n)
  MVF <- param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*d$FT))
...
zzzz_MVF_inv <- function(param,d) {
  n <- length(d$FN)
  r <- data.frame()
  cumFailTimes <- -(log((param$zzzz_aMLE-d$FN)/param$zzzz_aMLE))/param$zzzz_bMLE
...
zzzz_MTTF <- function(param,d) {
  ...
  IFTimes <- ((currentFailNums*currentTimes$FT)/(param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*currentTimes$FT)))) - ((prevFailNums*prevTimes$FT)/(param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*prevTimes$FT))))
  #  r[1,1] <- 1
  #  r[1,2] <- ((d$FT[1])/(param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*d$FT[1]))))
  #  for(i in 2:n){
  #    r[i,1] <- i
  #    r[i,2] <- ((i*d$FT[i])/(param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*d$FT[i])))) - (((i-1)*d$FT[i-1])/(param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*d$FT[i-1]))))
  #  }
  r <- data.frame(c(1, currentFailNums), c(((d$FT[1])/(param$zzzz_aMLE*(1-exp(-param$zzzz_bMLE*d$FT[1])))), IFTimes), rep("zzzz", n))
...
#  for(i in 1:n){
#    r[i,1] <- i
#    r[i,2] <-(1/(params$zzzz_aMLE*params$zzzz_bMLE*(exp(-params$zzzz_bMLE*d$FT[i]))))
#    r[i,3] <- "zzzz"
#    }
...
zzzz_FI <- function(param,d) {
  ...
  r <- data.frame()
  fail_number <- c(1:n)
  failIntensity <- param$zzzz_aMLE*param$zzzz_bMLE*exp(-param$zzzz_bMLE*d$FT)
  #  for(i in 1:length(fail_number)){
  #    r[i,1] <- fail_number[i]
  #    r[i,2] <- param$zzzz_aMLE*param$zzzz_bMLE*exp(-param$zzzz_bMLE*d$FT[i])
  #  }
...
#  for(i in 1:n){
#    r[i,1] <- d$FT[i]
#    r[i,2] <- params$zzzz_aMLE*params$zzzz_bMLE*(exp(-params$zzzz_bMLE*d$FT[i]))
#    r[i,3] <- "zzzz"
#    }
#  r <- data.frame(r[1],r[2],r[3])
#  names(r) <- c("Failure_Count","Failure_Rate","Model")
...
zzzz_R <- function(params,d){
  ...
  for(i in 1:n){
    r[i,1] <- d$FT[i]
    r[i,2] <- exp(-params$zzzz_bMLE*d$FT[i])
    r[i,3] <- "zzzz"
  }
  r <- data.frame(r[1],r[2],r[3])
...
zzzz_lnL <- function(x,params){
  ...
  firstSumTerm <- 0
  for(i in 1:n){
    firstSumTerm = firstSumTerm + (-params$zzzz_bMLE*x[i])
  }
  lnL <- -(params$zzzz_aMLE)*(1-exp(-params$zzzz_bMLE*tn)) + n*(log(params$zzzz_aMLE)) +n*log(params$zzzz_bMLE) + firstSumTerm
  lnL
}

zzzz_MVF_cont <- function(params,t){
  return(params$zzzz_aMLE*(1-exp(-params$zzzz_bMLE*t)))
}
...
zzzz_R_MLE_root <- function(params,cur_time,delta, reliability){
  root_equation <- reliability - exp(params$zzzz_aMLE*(1-exp(-params$zzzz_bMLE*cur_time)) -params$zzzz_aMLE*(1-exp(-params$zzzz_bMLE*(cur_time+delta))))
