...
Wei_lnL_new <- function(params,paramNames,negLnL,failData){
  ...
  n <- length(failData)
  tn <- failData[n]
  lnL <- n*log(as.list(params)$Wei_aMLE) - n*as.list(params)$Wei_aMLE*log(as.list(params)$Wei_bMLE) + (as.list(params)$Wei_aMLE-1)*sum(log(failData-rep(as.list(params)$Wei_cMLE,n))) - sum((failData-rep(as.list(params)$Wei_cMLE,n))/rep(as.list(params)$Wei_bMLE,n))^as.list(params)$Wei_aMLE
  if(negLnL == FALSE) {
    return(lnL)
  }
  else {
