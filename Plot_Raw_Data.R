...
if((DataIntervalEnd - DataIntervalStart + 1) >= K_minDataModelIntervalWidth) {
  ...
  if((length(grep("FT",names(input_data)))>0) || (length(grep("IF",names(input_data)))>0)) {
    ...
    FT <- c(unlist(subset(subset(FC_to_IF_data, FC_to_IF_data$FC_TI >= DataIntervalStart, select = c(FC_FN, FC_TI, FC_IF, FC_FT)), FC_to_IF_data$TI <= DataIntervalEnd, select = FC_FT)), use.names=FALSE)
    IF <- c(unlist(subset(subset(FC_to_IF_data, FC_to_IF_data$FC_TI >= DataIntervalStart, select = c(FC_FN, FC_TI, FC_IF, FC_FT)), FC_to_IF_data$TI <= DataIntervalEnd, select = FC_IF)), use.names=FALSE)
